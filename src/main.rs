...
use four::{random_in_unit_sphere, MaterialScatter, Mesh, Ray, Vec3};
use nalgebra::vector;
use rand::Rng;
...
fn main() {
  ...
  let sphere = Mesh {
    ...
    material: &Lambertian::new(vector![0.1, 0.2, 0.5]),
  };
  let sphere2 = Mesh {
    geometry: &Sphere {
    ...
    material: &Lambertian::new(vector![0.8, 0.8, 0.0]),
  };
  let sphere3 = Mesh {
    geometry: &Sphere {
    ...
    material: &Metal::new(vector![0.8, 0.6, 0.2], 0.0),
  };
  let sphere4 = Mesh {
    geometry: &Sphere {
      center: vector![-1.0, 0.0, -1.0],
  ...
  let r = (PI / 4.0).cos();
  let blue_ball = Mesh {
    geometry: &Sphere::new(vector![-r, 0., -1.], r),
    material: &Lambertian::new(vector![0., 0., 1.]),
  };
  let red_ball = Mesh {
    geometry: &Sphere::new(vector![r, 0., -1.], r),
    material: &Lambertian::new(vector![1., 0., 0.]),
  };
