...
impl Material for Dielectric {
    fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> Option<MaterialScatter> {
        let (outward_normal, ni_over_nt, cosine) = if r_in.direction.dot(&rec.normal) > 0.0 {
          (-rec.normal, self.ref_idx, self.ref_idx * r_in.direction.dot(&rec.normal) / r_in.direction.magnitude())
        } else {
          (rec.normal, 1.0 / self.ref_idx, -r_in.direction.dot(&rec.normal) / r_in.direction.magnitude())
        };
        ...
        return Some(MaterialScatter {
          attenuation: vector![1.0, 1.0, 1.0], // pure glass reflect all color, you can try to change [0.3, 0.3, 1.0] for blue glass
          scattered: Ray {
            origin: rec.position,
            direction: direction
          }
