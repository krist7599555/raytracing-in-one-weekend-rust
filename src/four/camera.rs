...
impl Camera {
  fn random_in_unit_disk() -> Vec3 {
    ...
    loop {
      let p = vector![rnd.gen_range(-1.0..1.0), rnd.gen_range(-1.0..1.0), 0.0];
      if p.dot(&p) >= 1.0 {
        return p;
      }
    }
  ...
  pub fn get_ray(&self, s: f32, t: f32) -> Ray {
    ...
    return Ray {
      ...
      direction: self.lower_left_coner + s * self.horizontal + t * self.vertical
        - self.origin
        - offset,
    };
  }
  ...
  /// `aperture` how blur is it. when `aperture=0` behavior is like pinhold (all
  /// pixel sharp)
  pub fn new(
    lookfrom: &Vec3, lookat: &Vec3, up: &Vec3, fovy: f32, aspect: f32, aperture: f32,
    focus_disk: f32,
  ) -> Self {
